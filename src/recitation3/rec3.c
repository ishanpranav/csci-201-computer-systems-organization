 ...
 * 
 * @param head a pointer to a pointer to a node structure; i.e., a head
 *             reference.
*/
void reverse(struct node **head)
{
...
void swapMiddle(struct node **head)
{
    ...
    struct node *next = middleRight->next;
    ...
    if (middleLeft == NULL) {
        ...
        *head = next;
        ...
        middleLeft->next = next;
    }

    // ... -> [middleRight] -> [middleLeft] -> [middleRight.next] -> [next]
    
    middleRight->next = next->next;
    next->next = middleRight;
}
